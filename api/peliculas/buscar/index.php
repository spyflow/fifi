...
function scrapePelisplus($query, $debug = false) {
    ...
    $cachedData = getCachedData($cacheKey, 'buscar');
    
    if ($cachedData !== null) {
        if ($debug) {
            responseJson(['source' => 'cache', 'data' => json_decode($cachedData, true)]);
        } else {
            responseJson(json_decode($cachedData, true));
        }
    }
    ...
    $result = ['movies' => $movies, 'series' => $series];
    
    // Cache the result for 15 days
    setCachedData($cacheKey, json_encode($result), 'buscar', 15);
...
$query = isset($_GET['q']) ? normalizeSearchQuery($_GET['q']) : '';
